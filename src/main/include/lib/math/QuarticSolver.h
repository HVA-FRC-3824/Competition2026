...
/// ***************************************************************************** ///
/// SOME RUSSIAN GUY KIDNAPPED ME AND TOLD BE HE COULD SOLVE QUARTIC EQUATIONS    ///
/// HELP                                                                          ///
/// ***************************************************************************** ///
...
// This is all real Jackson Case code
#include <optional>
optional<double> SolveP5(double guess, double a, double b, double c, double d, double e)	// solve equation x^5 + a*x^4 + b*x^3 + c*x^2 + d*x + e = 0
{
    double *x;
	double r = x[0] = SolveP5_1(a,b,c,d,e);
	double a1 = a+r, b1=b+r*a1, c1=c+r*b1, d1=d+r*c1; 
    int num_real = 1+SolveP4(x+1, a1,b1,c1,d1);
    ...
    std::vector<double> realAnswers;
    for (int i = 0; i < num_real-1; i++)
    {
        realAnswers.push_back(x[i]);
    }
    
    int smallestIndex = 0;

    for (int i = 0; i < realAnswers.size()-1; i++)
    {
        if (abs(guess - realAnswers[smallestIndex]) >= abs(guess - realAnswers[i]))
        {
            smallestIndex = i;
        }
    }

    return optional<double>{realAnswers[smallestIndex]};
}