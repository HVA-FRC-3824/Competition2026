...
frc2::CommandPtr TowerAimHub(Tower *tower)
{
    ...
    return frc2::InstantCommand{
        [=] () { tower->SetState(TowerState{TowerMode::ShootingToHub, 0_deg, 0_rpm, 0}); },  // Execution function
...
frc2::CommandPtr TowerIdle(Tower *tower)
{
    // Create and return a InstantCommand that aims the tower at the hub
    return frc2::InstantCommand{
        [=] () { tower->SetState(TowerState{TowerMode::Idle, 0_deg, 0_rpm, 0}); },  // Execution function
        {tower}  // Requirements (subsystems required by this command)
    }.ToPtr();
}
#pragma endregion
...
frc2::CommandPtr TowerAimPassZone(Tower *tower)
{
    ...
    return frc2::InstantCommand{
        [=] () { tower->SetState(TowerState{TowerMode::PassingToAdjacentZone, 0_deg, 0_rpm, 0}); },  // Execution function
        {tower}  // Requirements (subsystems required by this command)
    }.ToPtr();
}
#pragma endregion
...
frc2::CommandPtr TowerAutomatic(Tower *tower)
{
    // Create and return a InstantCommand that aims the tower at the adjacent pass zone
    return frc2::InstantCommand{
        [=] () { tower->SetState(TowerState{TowerMode::Automatic, 0_deg, 0_rpm, 0}); },  // Execution function
        {tower}  // Requirements (subsystems required by this command)
    }.ToPtr();
}
#pragma endregion
