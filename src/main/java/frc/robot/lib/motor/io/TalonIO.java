...
public class TalonIO implements MotorIO
{
  ...
  public void config(MotorConfig config) {
    ...
    talonConfig.MotorOutput.Inverted = config.isInverted()
      ? InvertedValue.Clockwise_Positive
      : InvertedValue.CounterClockwise_Positive;
    talonConfig.MotorOutput.NeutralMode = config.isBrakeMode()
    ...
    talonConfig.MotionMagic.MotionMagicCruiseVelocity = config.getVelocityLimit().in(RotationsPerSecond);
    talonConfig.MotionMagic.MotionMagicAcceleration   = config.getAccelerationLimit().in(RotationsPerSecondPerSecond);
  ...
  public void follow(int id, boolean inverted) {
    ...
    var status = m_motor.setControl(new Follower(id, inverted ? MotorAlignmentValue.Opposed : MotorAlignmentValue.Aligned));
    Logger.recordOutput("MotorErr/Talon " + m_motor.getDeviceID(), status.toString());
  }
