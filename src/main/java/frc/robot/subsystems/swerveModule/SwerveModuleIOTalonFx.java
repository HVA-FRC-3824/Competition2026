...
public class SwerveModuleIOTalonFx implements SwerveModuleIO
{
  ...
  public SwerveModuleIOTalonFx(int moduleNum, int driveMotorCanId, int angleMotorCanId, int angleEncoderCanId, Angle forwardsAngle) {

    m_drivingMotor     = new TalonIO(driveMotorCanId, Constants.Chassis.DriveConfig);
    m_angleMotor     = new TalonIO(angleMotorCanId, Constants.Chassis.TurnConfig);
  ...
  public void setWheelAngleToForward() {
    ...
    Angle moveDegrees = angleAbsoluteEncoder.getAbsolutePosition().waitForUpdate(0.1).getValue().minus(m_forwardsAngle);
